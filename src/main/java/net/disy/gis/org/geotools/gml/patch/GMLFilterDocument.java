...
public class GMLFilterDocument extends org.xml.sax.helpers.XMLFilterImpl {
  ...
  private static final List<String> SUB_GEOMETRY_TYPES = List.of("outerBoundaryIs", "innerBoundaryIs");

  /** Base geometry elements that may be passed in GML */
  private static final List<String> BASE_GEOMETRY_TYPES = List.of(
      "Point",
      "LineString",
      "Polygon",
      "LinearRing",
  ...
  private class CoordinateReader {
    ...
    public void readCoordinates(String coordinateString) throws SAXException {
      ...
      while (coordinateSets.hasMoreElements()) {
        ...
        x = Double.parseDouble(coordinates.nextToken().trim());
        y = Double.parseDouble(coordinates.nextToken().trim());

        if (coordinates.hasMoreElements()) {
          z = Double.parseDouble(coordinates.nextToken().trim());
          parent.gmlCoordinates(x, y, z);
        } else {
          parent.gmlCoordinates(x, y);
        }
      }
    }
    ...
    public void readCoord(String coordString) {
      ...
      switch (insideInnerFlag) {
        case INSIDE_X:
          x = Double.parseDouble(coordString.trim());

          break;

        case INSIDE_Y:
          y = Double.parseDouble(coordString.trim());

          break;

        case INSIDE_Z:
          z = Double.parseDouble(coordString.trim());
    ...
    public void insideCoordinates(boolean isInside, org.xml.sax.Attributes atts) {
      ...
      coordinateDelimeter = ",";
      decimalDelimeter = new StringBuffer(".");
      tupleDelimeter = " ";
      for (int i = 0; i < atts.getLength(); ++i) {
        String localName = atts.getLocalName(i);
        if (localName.equalsIgnoreCase("cs")) {
          coordinateDelimeter = atts.getValue(i);
        }
        if (localName.equalsIgnoreCase("decimal")) {
          decimalDelimeter = new StringBuffer(atts.getValue(i));
        }
        if (localName.equalsIgnoreCase("ts")) {
          tupleDelimeter = atts.getValue(i);
        }
      }
    }
